# Influitive Ansible Docker Image

Ansible Docker Image designed to execute ansible-playbook command against an externally mounted set of Ansible playbooks. Based on [walokra/docker-ansible-playbook](https://github.com/walokra/docker-ansible-playbook)
...
```
$ docker run --name ansible-playbook --rm influitive/ansible --version
...
```
docker run --rm -it -v PATH_TO_LOCAL_PLAYBOOKS_DIR:/ansible/playbooks influitive/ansible PLAYBOOK_FILE
```
...
```
docker run --rm -it -v $(pwd):/ansible/playbooks influitive/ansible site.yml
```
...
docker run --rm -it -v $(pwd):/ansible/playbooks --entrypoint ansible-vault \
  influitive/ansible encrypt FILENAME
```
